...
actual fun createHttpClient(tokenStorage: TokenStorage): HttpClient {
    ...
    return HttpClient(CIO) {
        ...
        install(ResponseObserver) {
            onResponse { response ->
                Log.d("KTOR_RESPONSE", "${response.status.value} for ${response.request.url}")
                Log.d("KTOR_RESPONSE_HEADERS", "${response.headers}")
                try {
                    val bodyText = response.bodyAsText()
                    if (bodyText.isNotEmpty()) {
                        Log.d("KTOR_RESPONSE_BODY", bodyText)
                    }
                } catch (e: Exception) {
                    Log.e("KTOR_RESPONSE_BODY_ERROR", e.message ?: "Unknown error")
                }
            }
        }
        install(Auth) {
...
private object RequestLogger : HttpClientPlugin<Unit, RequestLogger> {
    ...
    override fun install(plugin: RequestLogger, scope: HttpClient) {
        scope.plugin(HttpSend).intercept { request ->
            // Log request details
            Log.d("KTOR_REQUEST_URL", "${request.url}")
            Log.d("KTOR_REQUEST_METHOD", "${request.method}")

            val headersText = request.headers.entries()
                .flatMap { entry -> entry.value.map { "${entry.key}: $it" } }
                .joinToString("\n")

            Log.d("KTOR_REQUEST_HEADERS", "\n$headersText")
